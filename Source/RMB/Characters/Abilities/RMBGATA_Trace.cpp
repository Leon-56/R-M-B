#include "RMBGATA_Trace.h"


#include "AbilitySystemComponent.h"
#include "Abilities/GameplayAbility.h"

ARMBGATA_Trace::ARMBGATA_Trace()
{
...
void ARMBGATA_Trace::ResetSpread()
{
	bUseAimingSpreadMod = false;
	BaseSpread = 0.0f;
	AimingSpreadMod = 0.0f;
...
float ARMBGATA_Trace::GetCurrentSpread() const
{
...
void ARMBGATA_Trace::SetStartLocation(const FGameplayAbilityTargetingLocationInfo& InStartLocation)
{
	StartLocation = InStartLocation;
}

void ARMBGATA_Trace::SetShouldProduceTargetDataOnServer(bool bInShouldProduceTargetDataOnServer)
{
	ShouldProduceTargetDataOnServer = bInShouldProduceTargetDataOnServer;
}

void ARMBGATA_Trace::SetDestroyOnConfirmation(bool bInDestroyOnConfirmation)
{
	bDestroyOnConfirmation = bDestroyOnConfirmation;
}

void ARMBGATA_Trace::StartTargeting(UGameplayAbility* Ability)
{
...
void ARMBGATA_Trace::ConfirmTargetingAndContinue()
{
...
void ARMBGATA_Trace::CancelTargeting()
{
...
void ARMBGATA_Trace::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	DestroyReticleActors();
...
void ARMBGATA_Trace::Tick(float DeltaSeconds)
{
...
void ARMBGATA_Trace::LineTraceWithFilter(TArray<FHitResult>& OutHitResults, const UWorld* World, const FGameplayTargetDataFilterHandle FilterHandle, const FVector& Start, const FVector& End, FName ProfileName, const FCollisionQueryParams Params)
{
	check(World);
...
void ARMBGATA_Trace::AimWithPlayerController(const AActor* InSourceActor, FCollisionQueryParams Params, const FVector& TraceStart, FVector& OutTraceEnd, bool bIgnorePitch)
{
...
bool ARMBGATA_Trace::ClipCameraRayToAbilityRange(FVector CameraLocation, FVector CameraDirection, FVector AbilityCenter, float AbilityRange, FVector& ClippedPosition)
{
...
void ARMBGATA_Trace::StopTargeting()
{
	SetActorTickEnabled(false);
...
FGameplayAbilityTargetDataHandle ARMBGATA_Trace::MakeTargetData(const TArray<FHitResult>& HitResults) const
{
...
TArray<FHitResult> ARMBGATA_Trace::PerformTrace(AActor* InSourceActor)
{
...
AGameplayAbilityWorldReticle* ARMBGATA_Trace::SpawnReticleActor(FVector Location, FRotator Rotation)
{
	if (ReticleClass)
	{
...
void ARMBGATA_Trace::DestroyReticleActors()
{
